...
#type kill 1 to reset 429 error
...
class AramBot(discord.Client):
    ...
    def __init__(self, champ_snapshot_path, chrome_driver_path, *args,
                 **kwargs):
    ...
    def create_aram_snapshot(self, driver, champ_name):
        print(champ_name)
        driver.get(
            fr"https://app.mobalytics.gg/lol/champions/{champ_name}/aram-builds"
        )
        try:
        ...
        driver.execute_script(
            f"window.scrollTo(0, document.body.scrollHeight * {SCROLLSIZE})")
        driver.save_screenshot(
            os.path.join(self.champ_snapshot_path,
                         str(champ_name) + ".png"))
    ...
    def get_snapshots(self, driver):
        ...
        current_champ_list = lol_watcher.data_dragon.champions(
            champions_version)

        current_champ_list = [
            champ.lower() for champ in current_champ_list["data"]
        ]
    ...
    def get_mmr(self, name):
        ...
        mmr = driver.find_element(
            "xpath",
            r"/html/body/container[2]/container[2]/container[1]/container/wrapper[2]/div[1]/span[1]"
        ).text
        rank = driver.find_element(
            "xpath",
            r"/html/body/container[2]/container[2]/container[1]/container/wrapper[2]/div[3]"
        ).text
        driver.quit()
        if mmr != "N/A":
...
def run_bot(champ_snapshot_path, chrome_driver_path):
    ...
    async def on_message(message):
        ...
        #Kero#4827
        if message.channel.id == 994681128965386241 and len(
                message.content) != 0:
            ...
            if type_of_message == VALUES["LEAGUE"]:
                ...
                file_name = aram_message.aram_snapshot(
                    client.champ_list, client.champ_snapshot_path)
                print(file_name)
                try:
                ...
                await message.channel.send(file=discord.File(
                    aram_message.special_message(client.champ_snapshot_path)))
            ...
            elif type_of_message == VALUES["TIME"]:
                format = '%Y-%m-%d %H:%M %p'
                eastcoast_time = datetime.now(
                    pytz.timezone("US/Eastern")).strftime("%I:%M %p")
                central_time = datetime.now(
                    pytz.timezone("US/Central")).strftime("%I:%M %p")
                westcoast_time = datetime.now(
                    pytz.timezone("US/Pacific")).strftime("%I:%M %p")

                await message.channel.send(
                    f"East coast time: {eastcoast_time}\nCentral Time: {central_time}\nWest coast time: {westcoast_time}"
                )
            elif type_of_message == VALUES["INT"]:
                with open("intlist.txt") as f:
                    lines = f.readlines()
